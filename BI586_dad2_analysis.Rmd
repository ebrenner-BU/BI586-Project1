...
First, we have to download our data from [NCBI BioSample](https://www.ncbi.nlm.nih.gov/biosample?LinkName=bioproject_biosample_all&from_uid=628692). We chose 5 offshore and 5 inshore samples for our analysis. Now we can download all the sample number (starting with SRR) directly on NCB by using the "send to" feature with settings Format = "Accessions List" and Sort By = "Default order". Then save the accessions list with file name srafile.
